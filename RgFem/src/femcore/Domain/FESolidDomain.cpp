...
FESolidElement* FESolidDomain::FindElement(const Vector3d& y, double r[3])
{
    int i, j;
    int NE = Elements();
    Vector3d x[FEElement::MAX_NODES];
    for (i=0; i<NE; ++i)
    {
...
//! This function finds the element in which point y lies and returns
//! the isoparametric coordinates in r if an element is found
FESolidElement* FESolidDomain::FindReferenceElement(const Vector3d& y, double r[3])
{
	int NE = Elements();
	Vector3d x[FEElement::MAX_NODES];
	for (int i = 0; i<NE; ++i)
...
//-----------------------------------------------------------------------------
void FESolidDomain::ProjectToElement(FESolidElement& el, const Vector3d& p, double r[3])
{
...
//-----------------------------------------------------------------------------
bool FESolidDomain::ProjectToReferenceElement(FESolidElement& el, const Vector3d& p, double r[3])
{
	const int MN = FEElement::MAX_NODES;
	Vector3d rt[MN];
...
double FESolidDomain::defgrad(FESolidElement &el, Matrix3d &F, int n)
{
    // nodal points
...
//! Mult GradJ by F-1
Vector3d FESolidDomain::GradJ(FESolidElement &el, int n)
{
    // nodal points
    Vector3d r[FEElement::MAX_NODES];
    GetCurrentNodalCoordinates(el, r);
    ...
    for (int i = 0; i<neln; ++i)
    {
        ...
        Matrix3d gradgrad = (((dg[0][0] & g[0]) + (dg[0][1] & g[1]) + (dg[0][2] & g[2]))*Gri
...
Vector3d FESolidDomain::GradJp(FESolidElement &el, int n)
{
    // nodal points
    Vector3d r[FEElement::MAX_NODES];
...
//! The deformation gradient is returned in F and its determinant is the return
//! value of the function
double FESolidDomain::defgrad(FESolidElement &el, Matrix3d &F, int n, Vector3d* r)
{
	// calculate inverse jacobian
	//    double Ji[3][3];
	//    invjac0(el, Ji, n);
...
double FESolidDomain::defgrad(FESolidElement &el, Matrix3d &F, double r, double s, double t)
{
    // shape function derivatives
...
//! The deformation gradient is returned in F and its determinant is the return
//! value of the function
double FESolidDomain::defgradp(FESolidElement &el, Matrix3d &F, int n)
{
    // nodal coordinates
    Vector3d r[FEElement::MAX_NODES];
	GetPreviousNodalCoordinates(el, r);
...
//-----------------------------------------------------------------------------
double FESolidDomain::invjac0(const FESolidElement& el, double r, double s, double t, Matrix3d& J)
{
    double Ji[3][3];
    double J0 = invjac0(el, Ji, r, s, t);
    J = Matrix3d(Ji[0][0], Ji[0][1], Ji[0][2], Ji[1][0], Ji[1][1], Ji[1][2], Ji[2][0], Ji[2][1], Ji[2][2]);
    return J0;
}
...
//! integration point n. The inverse jacobian is retured in Ji
//! The return value is the determinant of the Jacobian (not the inverse!)
double FESolidDomain::invjact(FESolidElement& el, double Ji[3][3], int n, const Vector3d* rt)
{
	// calculate jacobian
	double J[3][3] = { 0 };
	int neln = el.NodeSize();
...
Vector3d FESolidDomain::gradient(FESolidElement& el, double* fn, int n)
{
    double Ji[3][3];
    invjact(el, Ji, n);
...
//-----------------------------------------------------------------------------
//! calculate gradient of function at integration points
Vector3d FESolidDomain::gradient(FESolidElement& el, int order, double* fn, int n)
{
	double Ji[3][3];
	invjact(el, Ji, n);
...
Matrix3d FESolidDomain::gradient(FESolidElement& el, Vector3d* fn, int n)
{
    double Ji[3][3];
    invjact(el, Ji, n);
				
    Vector3d g1(Ji[0][0],Ji[0][1],Ji[0][2]);
    Vector3d g2(Ji[1][0],Ji[1][1],Ji[1][2]);
    Vector3d g3(Ji[2][0],Ji[2][1],Ji[2][2]);
...
Matrix3d FESolidDomain::gradient(FESolidElement& el, Vector3d* fn, int n, const double alpha)
{
    double Ji[3][3];
    invjact(el, Ji, n, alpha);
...
//! calculate spatial gradient of function at integration points
//! at previous time
Matrix3d FESolidDomain::gradientp(FESolidElement& el, Vector3d* fn, int n)
{
    double Ji[3][3];
    invjactp(el, Ji, n);
...
void FESolidDomain::CoBaseVectors0(FESolidElement& el, int j, Vector3d g[3])
{
    // get the shape function derivatives
    double* Hr = el.Gr(j);
    double* Hs = el.Gs(j);
...
void FESolidDomain::CoBaseVectors(FESolidElement& el, int j, Vector3d g[3])
{
    // get the shape function derivatives
    double* Hr = el.Gr(j);
    double* Hs = el.Gs(j);
...
void FESolidDomain::CoBaseVectors(FESolidElement& el, int j, Vector3d g[3], const double alpha)
{
    // get the shape function derivatives
    double* Hr = el.Gr(j);
    double* Hs = el.Gs(j);
...
void FESolidDomain::ContraBaseVectors0(FESolidElement& el, int j, Vector3d gcnt[3])
{
    Vector3d gcov[3];
...
void FESolidDomain::ContraBaseVectors(FESolidElement& el, int j, Vector3d gcnt[3])
{
    Vector3d gcov[3];
...
void FESolidDomain::ContraBaseVectors(FESolidElement& el, int j, Vector3d gcnt[3], const double alpha)
{
    Vector3d gcov[3];
...
void FESolidDomain::CoBaseVectorDerivatives(FESolidElement& el, int j, Vector3d dg[3][3])
{
    FEMesh& m = *m_pMesh;
    ...
    dg[0][0] = dg[0][1] = dg[0][2] = Vector3d(0,0,0);  // derivatives of g[0]
    dg[1][0] = dg[1][1] = dg[1][2] = Vector3d(0,0,0);  // derivatives of g[1]
    dg[2][0] = dg[2][1] = dg[2][2] = Vector3d(0,0,0);  // derivatives of g[2]
...
void FESolidDomain::CoBaseVectorDerivatives0(FESolidElement& el, int j, Vector3d dg[3][3])
{
    FEMesh& m = *m_pMesh;
...
void FESolidDomain::CoBaseVectorDerivatives(FESolidElement& el, int j, Vector3d dg[3][3], const double alpha)
{
    FEMesh& m = *m_pMesh;
...
void FESolidDomain::ContraBaseVectorDerivatives(FESolidElement& el, int j, Vector3d dgcnt[3][3])
{
    Vector3d gcnt[3];
    Vector3d dgcov[3][3];
...
void FESolidDomain::ContraBaseVectorDerivatives0(FESolidElement& el, int j, Vector3d dgcnt[3][3])
{
    Vector3d gcnt[3];
    Vector3d dgcov[3][3];
...
void FESolidDomain::ContraBaseVectorDerivatives(FESolidElement& el, int j, Vector3d dgcnt[3][3], const double alpha)
{
    Vector3d gcnt[3];
    Vector3d dgcov[3][3];
...
Vector3d FESolidDomain::lapvec(FESolidElement& el, Vector3d* fn, int n)
{
    Vector3d gcnt[3];
    Vector3d dgcnt[3][3];
...
Vector3d FESolidDomain::gradivec(FESolidElement& el, Vector3d* fn, int n)
{
    Vector3d gcnt[3];
    Vector3d dgcnt[3][3];
    ContraBaseVectors(el, n, gcnt);
...
//-----------------------------------------------------------------------------
double FESolidDomain::ShapeGradient(FESolidElement& el, int n, Vector3d* GradH)
{
    // calculate jacobian
    double Ji[3][3];
...
//-----------------------------------------------------------------------------
double FESolidDomain::ShapeGradient(FESolidElement& el, int n, Vector3d* GradH, const double alpha)
{
...
//-----------------------------------------------------------------------------
double FESolidDomain::ShapeGradient0(FESolidElement& el, int n, Vector3d* GradH)
{
    // calculate jacobian
    double Ji[3][3];
...
//-----------------------------------------------------------------------------
double FESolidDomain::ShapeGradient(FESolidElement& el, double r, double s, double t, Vector3d* GradH)
{
    // calculate jacobian
    double Ji[3][3];
...
//-----------------------------------------------------------------------------
double FESolidDomain::ShapeGradient0(FESolidElement& el, double r, double s, double t, Vector3d* GradH)
{
    // calculate jacobian
    double Ji[3][3];
