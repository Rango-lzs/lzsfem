...
void FESolidSolver2::UpdateIncrements(vector<double>& Ui, vector<double>& ui, bool emap)
{
    ...
    for (int i = 0; i < mesh.Nodes(); ++i)
    {
        ...
        if ((n = node.m_dofs[m_dofU[0]]) >= 0)
            Ui[n] += ui[n];
        if ((n = node.m_dofs[m_dofU[1]]) >= 0)
            Ui[n] += ui[n];
        if ((n = node.m_dofs[m_dofU[2]]) >= 0)
            Ui[n] += ui[n];

        // rotational dofs
        if ((n = node.m_dofs[m_dofSQ[0]]) >= 0)
            Ui[n] += ui[n];
        if ((n = node.m_dofs[m_dofSQ[1]]) >= 0)
            Ui[n] += ui[n];
        if ((n = node.m_dofs[m_dofSQ[2]]) >= 0)
            Ui[n] += ui[n];

        // shell dofs
        if ((n = node.m_dofs[m_dofSU[0]]) >= 0)
            Ui[n] += ui[n];
        if ((n = node.m_dofs[m_dofSU[1]]) >= 0)
            Ui[n] += ui[n];
        if ((n = node.m_dofs[m_dofSU[2]]) >= 0)
            Ui[n] += ui[n];
    }
...
void FESolidSolver2::Update2(const vector<double>& ui)
{
    ...
    for (int i = 0; i < mesh.Nodes(); ++i)
    {
        ...
        if (nx >= 0)
            du.x = ui[nx];
        int ny = -node.m_dofs[m_dofU[1]] - 2;
        if (ny >= 0)
            du.y = ui[ny];
        int nz = -node.m_dofs[m_dofU[2]] - 2;
        if (nz >= 0)
            du.z = ui[nz];
        ...
        if (nbx >= 0)
            db.x = ui[nbx];
        int nby = -node.m_dofs[m_dofSU[1]] - 2;
        if (nby >= 0)
            db.y = ui[nby];
        int nbz = -node.m_dofs[m_dofSU[2]] - 2;
        if (nbz >= 0)
            db.z = ui[nbz];
...
bool FESolidSolver2::Quasin()
{
    ...
    do
    {
        ...
        // check for nans
        if (ISNAN(normR1))
            throw NANInResidualDetected();
        if (ISNAN(normu))
            throw NANInSolutionDetected();
...
public:
    ArcLengthFailed()
        : FEException("arc-length update has failed")
    {
    }
};
...
void FESolidSolver2::DoArcLength()
{
    ...
    if (m_arcLength == ARC_LENGTH_METHOD::CRISFIELD)
    {
        ...
        else
        {
            ...
            double c = m_ui * (m_Ui * 2.0 + m_ui) + m_Ui * m_Ui - m_al_ds * m_al_ds +
                       (psi * psi) * (m_al_inc * m_al_inc) * Fe_norm2;
...
bool FESolidSolver2::Residual(vector<double>& R)
{
    ...
    if (m_arcLength > 0)
    {
        ...
        for (int i = 0; i < R.size(); ++i)
            R[i] = m_Fext[i] * m_al_lam - m_Fint[i];
    }
...
void FESolidSolver2::ExternalForces(FEGlobalVector& RHS)
{
    ...
    */
    ...
    for (int i = 0; i < mesh.Nodes(); ++i)
    {
        ...
        int n;
        if ((n = node.m_dofs[m_dofU[0]]) >= 0)
            node.set_load(m_dofU[0], -m_Fr[n]);
        if ((n = -node.m_dofs[m_dofU[0]] - 2) >= 0)
            node.set_load(m_dofU[0], -m_Fr[n]);

        if ((n = node.m_dofs[m_dofU[1]]) >= 0)
            node.set_load(m_dofU[1], -m_Fr[n]);
        if ((n = -node.m_dofs[m_dofU[1]] - 2) >= 0)
            node.set_load(m_dofU[1], -m_Fr[n]);

        if ((n = node.m_dofs[m_dofU[2]]) >= 0)
            node.set_load(m_dofU[2], -m_Fr[n]);
        if ((n = -node.m_dofs[m_dofU[2]] - 2) >= 0)
            node.set_load(m_dofU[2], -m_Fr[n]);
