...
void FENewtonSolver::SetDefaultStrategy(QN_STRATEGY qn)
{
    ...
    {
        case QN_BFGS:
            p.SetDefaultType("BFGS");
            break;
        case QN_BROYDEN:
            p.SetDefaultType("Broyden");
            break;
        case QN_JFNK:
            p.SetDefaultType("JFNK");
            break;
        default:
            assert(false);
    }
...
bool FENewtonSolver::ReformStiffness()
{
    ...
    if (m_nref >= m_maxref)
        throw MaxStiffnessReformations();
...
{
    {
        ...
        else
        {
            ...
            if (parts > 1)
            {
                ...
                {
                    feLog("\t\tpartition %d ............................ : %d\n", i + 1,
                          m_plinsolve->GetPartitionSize(i));
                }
            }
        }
    }
...
bool FENewtonSolver::Quasin()
{
    ...
    // Initialize QN method, 计算初始残差, 初始刚度矩阵
    QNInit();
...
bool FENewtonSolver::CheckConvergence(int niter, const vector<double>& ui, double ls)
{
    ...
    // check convergence
    bool bconv = true;
    if ((m_Rtol > 0) && (m_residuNorm.norm > m_Rtol * m_residuNorm.norm0))
        bconv = false;
    if ((m_Etol > 0) && (m_energyNorm.norm > m_Etol * m_energyNorm.norm0))
        bconv = false;
    for (int i = 0; i < vars; ++i)
    {
        ConvergenceInfo& c = m_solutionNorm[i];
        if (c.IsConverged() == false)
            bconv = false;
    }
    ...
    feLog("\t   residual         %15le %15le %15le \n", m_residuNorm.norm0, m_residuNorm.norm,
          m_Rtol * m_residuNorm.norm0);
    feLog("\t   energy           %15le %15le %15le \n", m_energyNorm.norm0, m_energyNorm.norm,
          m_Etol * m_energyNorm.norm0);
    for (int i = 0; i < vars; ++i)
    {
        ConvergenceInfo& c = m_solutionNorm[i];
        FESolutionVariable& var = m_Var[c.nvar];
