...
FEParamValue FEParamValue::component(int n)
{
    switch (m_itype)
    {
        case FE_PARAM_VEC2D:
        {
            Vector2d& r = value<Vector2d>();
            if (n == 0)
                return FEParamValue(m_param, &r.x(), FE_PARAM_DOUBLE);
            if (n == 1)
                return FEParamValue(m_param, &r.y(), FE_PARAM_DOUBLE);
        }
        break;
        case FE_PARAM_VEC3D:
        {
            Vector3d& r = value<Vector3d>();
            if (n == 0)
                return FEParamValue(m_param, &r.x, FE_PARAM_DOUBLE);
            if (n == 1)
                return FEParamValue(m_param, &r.y, FE_PARAM_DOUBLE);
            if (n == 2)
                return FEParamValue(m_param, &r.z, FE_PARAM_DOUBLE);
        }
        break;
        case FE_PARAM_MAT3DS:
        {
            Matrix3ds& a = value<Matrix3ds>();
            if (n == 0)
                return FEParamValue(m_param, &a.xx(), FE_PARAM_DOUBLE);
            if (n == 1)
                return FEParamValue(m_param, &a.xy(), FE_PARAM_DOUBLE);
            if (n == 2)
                return FEParamValue(m_param, &a.yy(), FE_PARAM_DOUBLE);
            if (n == 3)
                return FEParamValue(m_param, &a.xz(), FE_PARAM_DOUBLE);
            if (n == 4)
                return FEParamValue(m_param, &a.yz(), FE_PARAM_DOUBLE);
            if (n == 5)
                return FEParamValue(m_param, &a.zz(), FE_PARAM_DOUBLE);
        }
        break;
    }
...
//-----------------------------------------------------------------------------
FEParam* FEParam::setUnits(const char* szunit)
{
    m_szunit = szunit;
    return this;
}
...
//-----------------------------------------------------------------------------
// parameter type
FEParamType FEParam::type() const
{
    return m_type;
}
    ...
    switch (m_type)
    {
        case FE_PARAM_DOUBLE:
        {
            if (i == -1)
            {
                ...
                return FEParamValue(this, &value<double>(i), FE_PARAM_DOUBLE);
        }
        break;
        case FE_PARAM_VEC3D:
        {
            ...
            {
                assert(m_dim == 1);
                return FEParamValue(this, &value<Vector3d>(), FE_PARAM_VEC3D);
            }
            else
                return FEParamValue(this, &value<Vector3d>(i), FE_PARAM_VEC3D);
        }
        break;
        case FE_PARAM_DOUBLE_MAPPED:
        {
            if (i == -1)
            {
                ...
                if (p.isConst())
                    return FEParamValue(this, &p.constValue(), FE_PARAM_DOUBLE);
                else
        ...
        case FE_PARAM_VEC3D_MAPPED:
        {
            if (i == -1)
            {
                ...
                if (p.isConst())
                    return FEParamValue(this, &p.constValue(), FE_PARAM_VEC3D);
                else
                    return FEParamValue(this, m_pv, m_type);
            }
            else
        ...
        case FE_PARAM_MAT3D_MAPPED:
        {
            if (i == -1)
            {
                ...
                if (p.isConst())
                    return FEParamValue(this, &p.constValue(), FE_PARAM_MAT3D);
                else
                    return FEParamValue(this, m_pv, m_type);
            }
            else
        ...
        {
            vector<Vector2d>& data = value<vector<Vector2d>>();
            if ((i >= 0) && (i < (int)data.size()))
       ...
       /* bool b = (m_watch ? *m_watch : false);
        ...
        if (m_dim == 1)
        {
            switch (m_type)
            {
                ...
                break;
                case FE_PARAM_STRING:
                    ar << (const char*)data_ptr();
                    break;
                case FE_PARAM_STD_STRING:
                    ar << value<string>();
                    break;
                case FE_PARAM_STD_VECTOR_VEC2D:
                    ar << value<std::vector<Vector2d>>();
                    break;
                case FE_PARAM_DOUBLE_MAPPED:
                {
                    FEParamDouble& p = value<FEParamDouble>();
...
//-----------------------------------------------------------------------------
void FEParam::setOwner(FEParamObject* pc)
{
    m_parent = pc;
}
FEParamObject* FEParam::owner() const
{
    return m_parent;
}
...
{
    if (b)
        m_flag = (m_flag | FEParamFlag::FE_PARAM_VOLATILE);
    else
        m_flag = (m_flag & ~FEParamFlag::FE_PARAM_VOLATILE);
    return this;
}
...
{
    if (b)
        m_flag = (m_flag | FEParamFlag::FE_PARAM_TOPLEVEL);
    else
        m_flag = (m_flag & ~FEParamFlag::FE_PARAM_TOPLEVEL);
    return this;
}
...
//-----------------------------------------------------------------------------
FEParam* FEParam::SetFlags(unsigned int flags)
{
    m_flag = flags;
    return this;
}
unsigned int FEParam::GetFlags() const
{
    return m_flag;
}
...
FEParamValue GetParameterComponent(const std::string& paramName, FEParam* param)
{
    ...
    {
        Vector3d* v = param->pvalue<Vector3d>(0);
        assert(v);
        if (v)
        {
            if (paramName == "x")
                return FEParamValue(param, &v->x, FE_PARAM_DOUBLE);
            else if (paramName == "y")
                return FEParamValue(param, &v->y, FE_PARAM_DOUBLE);
            else if (paramName == "z")
                return FEParamValue(param, &v->z, FE_PARAM_DOUBLE);
            else
                return FEParamValue();
        }
        else
