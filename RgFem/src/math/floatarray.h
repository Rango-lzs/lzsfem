...
namespace fem
{
	...
	public:
...
#ifndef NDEBUG
			if (i >= this->giveSize()) {
				FEM_ERROR("array error on index : %d >= %d", i, this->giveSize());
			}
#endif
			return values[i];
		}
		...
		{
			if (i <= 0) {
				FEM_ERROR("array error on index : %d <= 0", i);
			}
	...
	FEM_EXPORT FloatArray& operator *= (FloatArray& x, const double& a);
	FEM_EXPORT FloatArray operator *(const double& a, const FloatArray& x);
	FEM_EXPORT FloatArray operator *(const FloatArray& x, const double& a);
	FEM_EXPORT FloatArray operator + (const FloatArray& x, const FloatArray& y);
	FEM_EXPORT FloatArray operator - (const FloatArray& x, const FloatArray& y);
	FEM_EXPORT FloatArray& operator += (FloatArray& x, const FloatArray& y);
	FEM_EXPORT FloatArray& operator -= (FloatArray& x, const FloatArray& y);

	FEM_EXPORT double norm(const FloatArray& x);
	FEM_EXPORT double norm_square(const FloatArray& x);
	FEM_EXPORT double dot(const FloatArray& x, const FloatArray& y);
	FEM_EXPORT double distance(const FloatArray& x, const FloatArray& y);
	FEM_EXPORT double distance_square(const FloatArray& x, const FloatArray& y);
	FEM_EXPORT bool isfinite(const FloatArray& x);
	FEM_EXPORT bool iszero(const FloatArray& x);
	FEM_EXPORT double sum(const FloatArray& x);
	FEM_EXPORT double product(const FloatArray& x);
	//@}
} // namespace fem
#endif // floatarray_h
