...
extern "C"
{
    ...
    extern void dgecon_(const char* norm, const int* n, const double* a, const int* lda, const double* anorm,
                        double* rcond, double* work, int* iwork, int* info, int norm_len);
    ...
    extern int dgesv_(const int* n, const int* nrhs, double* a, const int* lda, int* ipiv, const double* b,
    ...
    extern int dsyevx_(const char* jobz, const char* range, const char* uplo, const int* n, double* a, const int* lda,
                       ...
                       double* w, double* z, const int* ldz, double* work, int* lwork, int* iwork, int* ifail,
                       int* info, int jobz_len, int range_len, int uplo_len);
    /// Solves system which has been LU-factorized.
    extern void dgetrs_(const char* trans, const int* n, const int* nrhs, double* a, const int* lda, int* ipiv,
    ...
    extern void dgemm_(const char* transa, const char* transb, const int* m, const int* n, const int* k,
                       ...
                       const double* beta, double* c, const int* ldc, int a_columns, int b_columns, int c_columns);
    ...
    extern void dger_(const int* m, const int* n, const double* alpha, const double* x, const int* incx,
                      const double* y, const int* incy, double* a, const int* lda, int x_len, int y_len, int a_columns);
    /// Symmetric dyad product of vector
    extern void dsyr_(const char* uplo, const int* n, const double* alpha, const double* x, const int* incx, double* a,
                      const int* lda, int x_len, int a_columns);
    /// Y = Y + alpha * X
...
namespace fem
{
    ...
    FloatMatrix ::FloatMatrix(std ::initializer_list<std ::initializer_list<double>> mat)
    {
        ...
        for (auto col : mat)
        {
            ...
            {
                FEM_ERROR("Initializer list has inconsistent column sizes.");
            }
#endif
            for (auto x : col)
            {
    ...
    // Checks that the receiver includes a position (i,j).
    {
        if (i <= 0)
        {
            FEM_ERROR("matrix error on rows : %d < 0", i);
        }
        if (j <= 0)
        {
            FEM_ERROR("matrix error on columns : %d < 0", j);
        }

        if (i > nRows)
        {
            FEM_ERROR("matrix error on rows : %d > %d", i, nRows);
        }

        if (j > nColumns)
        {
            printf("APA \n");
            FEM_ERROR("matrix error on columns : %d > %d", j, nColumns);
        }
    }
    ...
    void FloatMatrix ::assemble(const FloatMatrix& src, const IntArray& loc)
    {
        ...
        {
            FEM_ERROR("dimensions of 'src' and 'loc' mismatch");
        }

        if (!src.isSquare())
        {
            FEM_ERROR("'src' is not sqaure matrix");
        }
#endif

        // loc, 为单元局部自由度对应的全局自由度
    ...
    void FloatMatrix ::assemble(const FloatMatrix& src, const IntArray& rowind, const IntArray& colind)
    {
        ...
        {
            FEM_ERROR("row dimensions of 'src' and 'rowind' mismatch");
        }

        if (nc != colind.giveSize())
        {
            FEM_ERROR("column dimensions of 'src' and 'colind' mismatch");
        }
#endif
    ...
    // Receiver = aMatrix * bMatrix
    {
        ...
        {
            FEM_ERROR("error in product A*B : dimensions do not match, A(*,%d), B(%d,*)", aMatrix.nColumns,
                      bMatrix.nRows);
        }
#endif
        ...
        dgemm_("n", "n", &this->nRows, &this->nColumns, &aMatrix.nColumns, &alpha, aMatrix.givePointer(),
               &aMatrix.nRows, bMatrix.givePointer(), &bMatrix.nRows, &beta, this->givePointer(), &this->nRows,
    ...
    // Receiver = aMatrix^T * bMatrix
    {
        ...
        {
            FEM_ERROR("error in product A*B : dimensions do not match");
        }
#endif
        RESIZE(aMatrix.nColumns, bMatrix.nColumns);
#ifdef __LAPACK_MODULE
        double alpha = 1., beta = 0.;
        dgemm_("t", "n", &this->nRows, &this->nColumns, &aMatrix.nRows, &alpha, aMatrix.givePointer(), &aMatrix.nRows,
               bMatrix.givePointer(), &bMatrix.nRows, &beta, this->givePointer(), &this->nRows, aMatrix.nColumns,
               bMatrix.nColumns, this->nColumns);
#else
...
#ifdef __LAPACK_MODULE
        double alpha = 1., beta = 0.;
        dgemm_("n", "t", &this->nRows, &this->nColumns, &aMatrix.nColumns, &alpha, aMatrix.givePointer(),
               &aMatrix.nRows, bMatrix.givePointer(), &bMatrix.nRows, &beta, this->givePointer(), &this->nRows,
               aMatrix.nColumns, bMatrix.nColumns, this->nColumns);
#else
        for (int i = 1; i <= aMatrix.nRows; i++)
    ...
    // Receiver = aMatrix * bMatrix
    {
        ...
        {
            FEM_ERROR("error in product receiver : dimensions do not match");
        }
#endif
    ...
    {  // normal should be at the first position, easier for interface material models
        ...
        {
            FEM_ERROR("Normal needs 1 to 3 components.");
        }
    }
    ...
    void FloatMatrix ::setSubMatrix(const FloatMatrix& src, int sr, int sc)
    {
        ...
        {
            FEM_ERROR("Sub matrix doesn't fit inside allocated space.");
        }
#endif
    ...
    void FloatMatrix ::copyColumn(FloatArray& dest, int c) const
    {
        ...
        {
            FEM_ERROR("Column outside range (%d)", c);
        }
#endif
...
#ifdef __LAPACK_MODULE
        double beta = 1.;
        ///@todo We should determine which is the best choice overall. For large systems more block matrix operations is
        /// necessary.
        /// For smaller systems the overhead from function calls might be larger, but the overhead might be tiny, or
        /// using symmetry at all might be undesireable.
        if (this->nRows < 20)
        {
            ...
            dgemm_("t", "n", &s1, &s1, &a.nRows, &dV, a.givePointer(), &a.nRows, b.givePointer(), &b.nRows, &beta,
                   this->givePointer(), &this->nRows, a.nColumns, b.nColumns, this->nColumns);
            // Second column block starting a memory position c * nRows
            dgemm_("t", "n", &this->nRows, &s2, &a.nRows, &dV, a.givePointer(), &a.nRows,
                   &b.givePointer()[s1 * b.nRows], &b.nRows, &beta, &this->givePointer()[s1 * this->nRows],
                   &this->nRows, a.nColumns, b.nColumns, this->nColumns);
        }
        else
        {
            // Get suitable blocksize. Around 10 rows should be suitable (slightly adjusted to minimize number of
            // blocks): Smaller blocks than ~10 didn't show any performance gains in my benchmarks. / Mikael
            ...
            {
                int s = end - start;
                dgemm_("t", "n", &end, &s, &a.nRows, &dV, a.givePointer(), &a.nRows, &b.givePointer()[start * b.nRows],
                       &b.nRows, &beta, &this->givePointer()[start * this->nRows], &this->nRows, a.nColumns, b.nColumns,
                       this->nColumns);
                start = end;
                end += block;
                if (end > this->nRows)
    ...
    void FloatMatrix ::plusDyadSymmUpper(const FloatArray& a, double dV)
    {
        ...
        dsyr_("u", &sizeA, &dV, a.givePointer(), &inc, this->givePointer(), &this->nRows, sizeA, this->nColumns);
#else
        for (int i = 1; i <= nRows; i++)
        {
            for (int j = i; j <= nColumns; j++)
...
#ifdef __LAPACK_MODULE
        double beta = 1.;
        dgemm_("t", "n", &this->nRows, &this->nColumns, &a.nRows, &dV, a.givePointer(), &a.nRows, b.givePointer(),
               &b.nRows, &beta, this->givePointer(), &this->nRows, a.nColumns, b.nColumns, this->nColumns);
#else
        for (int i = 1; i <= nRows; i++)
        {
    ...
    void FloatMatrix ::plusDyadUnsym(const FloatArray& a, const FloatArray& b, double dV)
    {
        ...
        dger_(&sizeA, &sizeB, &dV, a.givePointer(), &inc, b.givePointer(), &inc, this->givePointer(), &this->nRows,
    ...
    // Receiver becomes inverse of given parameter src. If necessary, size is adjusted.
    {
        ...
        if (!src.isSquare())
        {
            FEM_ERROR("cannot inverse a %d by %d matrix", src.nRows, src.nColumns);
        }
#endif
        ...
        else
        {
            ...
            *this = src;
            ...
            dgetri_(&this->nRows, this->givePointer(), &this->nRows, ipiv.givePointer(), work.givePointer(), &lwork,
                    &info);
            if (info > 0)
            {
            ...
            {
                FEM_ERROR("Error on input %d", info);
            }
#else
            ...
            for (int i = 1; i < nRows; i++)
            {
                ...
                if (fabs(piv) < 1.e-30)
                {
                    FEM_WARNING("pivot (%d,%d) to close to small (< 1.e-20)", i, i);
                    return false;
                }
    ...
    void FloatMatrix ::beSubMatrixOf(const FloatMatrix& src, int topRow, int bottomRow, int topCol, int bottomCol)
    /*
     ...
     */
    {
        ...
        {
            FEM_ERROR("subindexes size mismatch");
        }
    ...
    void FloatMatrix ::beSubMatrixOf(const FloatMatrix& src, const IntArray& indxRow, const IntArray& indxCol)
    /*
     ...
     */
    {
        ...
        {
            FEM_ERROR("index exceeds source dimensions");
        }
#endif
    ...
    // adjusts its size to that of aMatrix. Returns the modified receiver.
    {
        ...
        {
            FEM_ERROR("dimensions mismatch : (r1,c1)+(r2,c2) : (%d,%d)+(%d,%d)", nRows, nColumns, aMatrix.nRows,
                      aMatrix.nColumns);
        }
#endif
    ...
    // solves equation b = this * x
    {
        ...
        if (!this->isSquare())
        {
            FEM_ERROR("cannot solve a %d by %d matrix", nRows, nColumns);
        }

        if (nRows != b.giveSize())
        {
            FEM_ERROR("dimension mismatch");
        }
#endif
        ...
        answer = b;
        // dgesv_( &this->nRows, &nrhs, this->givePointer(), &this->nRows, ipiv.givePointer(), answer.givePointer(),
        // &this->nRows, &info );
        dgetrf_(&this->nRows, &this->nRows, this->givePointer(), &this->nRows, ipiv.givePointer(), &info);
        if (info == 0)
        {
            dgetrs_(transpose ? "Transpose" : "No transpose", &this->nRows, &nrhs, this->givePointer(), &this->nRows,
                    ipiv.givePointer(), answer.givePointer(), &this->nRows, &info);
        }
        if (info != 0)
        {
            return false;
    ...
    //
    {
        ...
        if (info == 0)
        {
            dgetrs_(transpose ? "t" : "n", &this->nRows, &answer.nColumns, this->givePointer(), &this->nRows,
                    ipiv.givePointer(), answer.givePointer(), &this->nRows, &info);
        }
        if (info != 0)
        {
            return false;  // FEM_ERROR("error %d", info);
        }
#else
        int pivRow, nPs;
        double piv, linkomb, help;
        ...
        for (int i = 1; i < nRows; i++)
        {
            ...
            if (fabs(piv) < 1.e-20)
            {
                return false;  // FEM_ERROR("pivot too small, cannot solve %d by %d matrix", nRows, nColumns);
            }
    ...
    void FloatMatrix ::beUnitMatrix()
    {
        ...
        if (!this->isSquare())
        {
            FEM_ERROR("cannot make unit matrix of %d by %d matrix", nRows, nColumns);
        }
#endif
    ...
    // Returns the determinant of the receiver.
    {
        ...
        if (!this->isSquare())
        {
            FEM_ERROR("cannot compute the determinant of a non-square %d by %d matrix", nRows, nColumns);
        }
#endif
        ...
        {
            FEM_ERROR("sorry, cannot compute the determinant of a matrix larger than 3x3");
        }
    ...
    double FloatMatrix ::giveTrace() const
    {
        ...
        if (!this->isSquare())
        {
            FEM_ERROR("cannot compute the trace of a non-square %d by %d matrix", nRows, nColumns);
        }
#endif
        double answer = 0.;
    ...
    // Prints the receiver to file.
    {
        ...
        {
            FEM_ERROR("Failed to write to file");
        }
    }
    ...
    // Initializes the lower half of the receiver to the upper half.
    {
        ...
        if (nRows != nColumns)
        {
            FEM_ERROR("cannot symmetrize a non-square matrix");
        }
    ...
    void FloatMatrix ::beMatrixForm(const FloatArray& aArray)
    {
        ...
        {
            FEM_ERROR("matrix dimension is not 3x3");
        }
#endif
        this->resize(3, 3);
    ...
    void FloatMatrix ::changeComponentOrder()
    {
        ...
        //         FEM_ERROR("matrix dimension is not 6x6");
        //     }
        // #  endif
    ...
    double FloatMatrix ::computeReciprocalCondition(char p) const
    {
        ...
        if (!this->isSquare())
        {
            FEM_ERROR("receiver must be square (is %d by %d)", this->nRows, this->nColumns);
        }
#endif
        ...
        if (n > 3)
        {  // Only use this routine for larger matrices. (not sure if it's suitable for n = 3) // Mikael
            FloatMatrix a_cpy = *this;
            dgetrf_(&n, &n, a_cpy.givePointer(), &n, iwork.givePointer(), &info);
            if (info < 0)
            {
                FEM_ERROR("dgetfr error %d\n", info);
            }
            dgecon_(&p, &(this->nRows), a_cpy.givePointer(), &this->nRows, &anorm, &rcond, work.givePointer(),
                    iwork.givePointer(), &info, 1);
            if (info < 0)
            {
                FEM_ERROR("dgecon error %d\n", info);
            }
            return rcond;
        }
#endif
...
bool FloatMatrix :: computeEigenValuesSymmetric(FloatArray &lambda, FloatMatrix &v, int neigs) const
{
    ...
    //{
    //    return buff.givePackSizeOfInt(1) + buff.givePackSizeOfInt(1) + buff.givePackSizeOfDouble(nRows * nColumns);
    //}
    ...
    bool FloatMatrix ::jaco_(FloatArray& eval, FloatMatrix& v, int nf)
    {
        ...
        for (int i = 1; i <= neq; i++)
        {
            for (int j = i + 1; j <= neq; j++)
            {
                ...
                {
                    FEM_ERROR("Not Symmetric matrix");
                }
            }
        }
...
#ifdef _BOOSTPYTHON_BINDINGS
    void FloatMatrix ::__setitem__(boost ::python ::api ::object t, double val)
    {
        this->at(boost ::python ::extract<int>(t[0]) + 1, boost ::python ::extract<int>(t[1]) + 1) = val;
    }

    double FloatMatrix ::__getitem__(boost ::python ::api ::object t)
    {
