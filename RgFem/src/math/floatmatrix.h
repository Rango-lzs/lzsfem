...
#ifndef _FLOAT_MATRIX_H
#define _FLOAT_MATRIX_H
...
namespace fem
{
    ...
    // Matrix stored in one dimension
    class FEM_EXPORT FloatMatrix
    {
    private:
        /// Number of rows.
        int nRows;
    ...
    public:
        ...
        inline int giveNumberOfRows() const
        {
            return nRows;
        }
        /// Returns number of columns of receiver.
        inline int giveNumberOfColumns() const
        {
            return nColumns;
        }
        ...
        /**
         * Solves the  system of linear equations @f$ K\cdot a = b @f$ . Uses Gaussian elimination with pivoting
        ...
        /**
         * Adds to the receiver the product @f$ a^{\mathrm{T}}\cdot b \mathrm{d}V @f$. If the receiver has zero size, it
         * is expanded. Assumes that receiver and product @f$ a^{\mathrm{T}}\cdot b \mathrm{d}V @f$ are symmetric
         * matrices. Computes only the upper half of receiver.
        ...
        /**
         * Adds to the receiver the dyadic product @f$ a \otimes a \mathrm{d}V @f$. If the receiver has zero size, it is
         * expanded. Computes only the upper half of receiver.
        ...
        /**
         * Adds to the receiver the product @f$a \otimes b \mathrm{d}V@f$. If the receiver has zero size, it is
         * expanded.
         * @param a Array a in equation.
         ...
         */
        inline const double* givePointer() const
        {
            return values.data();
        }
        inline double* givePointer()
        {
            return values.data();
        }
        ...
        void beCopyOf(const FloatMatrix& src)
        {
            this->operator=(src);
        }
#endif
    ...
    FEM_EXPORT FloatMatrix& operator*=(FloatMatrix& x, const double& a);
    FEM_EXPORT FloatMatrix operator*(const FloatMatrix& a, const FloatMatrix& b);
    FEM_EXPORT FloatArray operator*(const FloatMatrix& a, const FloatArray& b);
    FEM_EXPORT FloatMatrix operator+(const FloatMatrix& a, const FloatMatrix& b);
    FEM_EXPORT FloatMatrix operator-(const FloatMatrix& a, const FloatMatrix& b);
    FEM_EXPORT FloatMatrix& operator+=(FloatMatrix& a, const FloatMatrix& b);
    FEM_EXPORT FloatMatrix& operator-=(FloatMatrix& a, const FloatMatrix& b);

    //@}

}  // end namespace fem
#endif  // _FLOAT_MATRIX_H
