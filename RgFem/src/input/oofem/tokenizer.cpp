...
namespace fem {
	...
	std::string Tokenizer::readStringToken(std::size_t& pos, const std::string& line)
	{
		pos++;
		...
		if (line[pos] == '"') {
			...
			FEM_WARNING("Missing closing separator (\") inserted at end of line");
		}
		return x;
	}


	std::string Tokenizer::readStructToken(std::size_t& pos, const std::string& line)
	{
		std::string x = this->readToken(pos, line, '}'); // read everything up to terminating '}' (or to the end of the string)
		if (line[pos] == '}') {
			pos++;            // check if terminating '}' was found
	...
	std::string Tokenizer::readSimpleExpressionToken(std::size_t& pos, const std::string& line)
	{
		pos++;
		std::string x = this->readToken(pos, line, '$'); // read everything up to terminating '$' (or to the end of the string)
		if (line[pos] == '$') {
	...
	std::string Tokenizer::readSimpleToken(std::size_t& pos, const std::string& line)
	{
	...
	std::string Tokenizer::readToken(std::size_t& pos, const std::string& line, char sep)
	{
		std::size_t startpos = pos;
