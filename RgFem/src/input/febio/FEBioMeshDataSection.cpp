...
void FEBioMeshDataSection::ParseMaterialAxes(XMLTag& tag, FEElementSet& set)
{
	...
	else
	{
		...
		//FEMappedValueMat3d* val = RANGO_NEW<EMappedValueMat3d>( GetFEModel(),"");
		//val->setDataMap(map);
		////pQ->SetProperty(val);
		//mesh.AddDataMap(map);
	}
...
void FEBioMeshDataSection::ParseMaterialData(XMLTag& tag, FEElementSet& set, const std::string& pname)
{
	...
	if (p->type() == FE_PARAM_DOUBLE_MAPPED)
	{
		...
		FEMappedValue* val = RANGO_NEW<FEMappedValue>( & fem,"");
		if (val == nullptr)
		{
	...
	else if (p->type() == FE_PARAM_MAT3DS_MAPPED)
	{
		...
		FEMappedValueMat3ds* val = RANGO_NEW<FEMappedValueMat3ds>(& fem,"");
		if (val == nullptr)
		{
			printf("Something went horribly wrong.");
			return;
		...
		if (fmt == FMT_ITEM)
		{
			...
			{
				double* d = data[i].val;
				Matrix3ds m(d[0], d[1], d[2], d[3], d[4], d[5]);
				map->set<Matrix3ds>(i, m * scale);
			}
	...
	else if (dataType == FE_VEC3D)
	{
		...
		do
		{
			if (tag == sztag)
			{
				...
				map.setValue(nid - 1, Vector3d(v[0], v[1], v[2]));
			}
			else throw XMLReader::InvalidTag(tag);
			++tag;
		} while (!tag.isend());
