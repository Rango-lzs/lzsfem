...
FEBioFileSection::FEBioFileSection(FEBioImport* feb)
    : FEFileSection(feb)
{
}

FEBioImport* FEBioFileSection::GetFEBioImport()
{
    return static_cast<FEBioImport*>(GetFileReader());
}
...
bool FEBioImport::Load(FEModel& fem, const char* szfile)
{
    ...
    try
    {
        ...
        if (b == false)
            return errf("FAILED building FEBio model.");
    }
...
bool FEBioImport::ReadFile(const char* szfile, bool broot)
{
    ...
    if (xml.Open(szfile) == false)
        return errf("FATAL ERROR: Failed opening input file %s\n\n", szfile);

    // Find the root element
    XMLTag tag;
    try
    {
        if (xml.FindTag("febio_spec", tag) == false)
            return errf("FATAL ERROR: febio_spec tag was not found. This is not a valid input file.\n\n");
    }
    catch (...)
    {
    ...
    try
    {
        ...
        if ((nversion != 0x0102) && (nversion != 0x0200) && (nversion != 0x0205) && (nversion != 0x0300) &&
            (nversion != 0x0400))
        ...
        // That means that model components that depend on DOFs can be defined before the Module tag (e.g. in multi-step
        // analyses) and this leads to problems. In 2.5 this is solved by requiring that the Module tag is defined at
        // the top of the file.
        if (broot && (nversion < 0x0205))
        {
            // We need to define a default Module type since before 2.5 this tag is optional for structural mechanics
            // model definitions.
        ...
        do
        {
            ...
            if (nversion >= 0x0200)
            {
                ...
                if (szinc)
                {
                    ...
                    if (tag.isleaf() == false)
                        return errf("FATAL ERROR: included sections may not have child sections.\n\n");

                    // read this section from an included file.
                    XMLReader xml2;
                    if (xml2.Open(szinc) == false)
                        return errf("FATAL ERROR: failed opening input file %s\n\n", szinc);

                    // find the febio_spec tag
                    XMLTag tag2;
                    if (xml2.FindTag("febio_spec", tag2) == false)
                        return errf("FATAL ERROR: febio_spec tag was not found. This is not a valid input file.\n\n");

                    // find the section we are looking for
                    char sz[512] = {0};
                    sprintf(sz, "febio_spec/%s", tag.Name());
                    if (xml2.FindTag(sz, tag2) == false)
                        return errf("FATAL ERROR: Couldn't find %s section in file %s.\n\n", tag.Name(), szinc);
...
//-----------------------------------------------------------------------------
void FEBioImport::SetDumpfileName(const char* sz)
{
    sprintf(m_szdmp, "%s", sz);
}
void FEBioImport::SetLogfileName(const char* sz)
{
    sprintf(m_szlog, "%s", sz);
}
void FEBioImport::SetPlotfileName(const char* sz)
{
    sprintf(m_szplt, "%s", sz);
}
...
FENodeSet* FEBioImport::ParseNodeSet(XMLTag& tag, const char* szatt)
{
    ...
    if (szset)
    {
        ...
        if (pns == 0)
            throw XMLReader::InvalidAttributeValue(tag, szatt, szset);
    }
    else
    {
        ...
        if (tag.isleaf())
        {
            ...
            for (int i = 0; i < l.size(); ++i)
                pns->Add(GetBuilder()->FindNodeFromID(l[i]));
        }
        else
        {
            // read the nodes
            ...
            do
            {
                ...
                else if (tag == "node_list")
                {
                    ...
                    for (int i = 0; i < nl.size(); ++i)
                        pns->Add(GetBuilder()->FindNodeFromID(nl[i]));
                }
                else
                    throw XMLReader::InvalidTag(tag);
                ++tag;
...
FESurface* FEBioImport::ParseSurface(XMLTag& tag, const char* szatt)
{
    ...
    //    if (ps)
    //    {
    //        if (GetBuilder()->BuildSurface(*psurf, *ps) == false)
    //            throw XMLReader::InvalidTag(tag);
    //    }
    //    else
    //        throw XMLReader::InvalidAttributeValue(tag, "set", szset);
    //}
    //else
    //{
    ...
    //        if (tag == "quad4")
    //            el.SetType(FE_QUAD4G4);
    //        else if (tag == "tri3")
    //            el.SetType(feb->m_ntri3);
    //        else if (tag == "tri6")
    //            el.SetType(feb->m_ntri6);
    //        else if (tag == "tri7")
    //            el.SetType(feb->m_ntri7);
    //        else if (tag == "tri10")
    //            el.SetType(feb->m_ntri10);
    //        else if (tag == "quad8")
    //            el.SetType(FE_QUAD8G9);
    //        else if (tag == "quad9")
