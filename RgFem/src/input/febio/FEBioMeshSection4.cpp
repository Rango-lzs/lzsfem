...
//-----------------------------------------------------------------------------
FEBioMeshSection4::FEBioMeshSection4(FEBioImport* pim)
...
void FEBioMeshSection4::Parse(XMLTag& tag)
{
    ...
    {
        if (tag == "Nodes")
            ParseNodeSection(tag, part);
        else if (tag == "Elements")
            ParseElementSection(tag, part);
        else if (tag == "NodeSet")
            ParseNodeSetSection(tag, part);
        else if (tag == "Surface")
            ParseSurfaceSection(tag, part);
        else if (tag == "Edge")
            ParseEdgeSection(tag, part);
        else if (tag == "ElementSet")
            ParseElementSetSection(tag, part);
        else if (tag == "PartList")
            ParsePartListSection(tag, part);
        else if (tag == "SurfacePair")
            ParseSurfacePairSection(tag, part);
        else if (tag == "DiscreteSet")
            ParseDiscreteSetSection(tag, part);
        else
...
void FEBioMeshSection4::ParseNodeSetSection(XMLTag& tag, FEBModel::Part* part)
{
    ...
    if (set)
        throw FEBioImport::RepeatedNodeSet(szname);
...
void FEBioMeshSection4::ParseSurfaceSection(XMLTag& tag, FEBModel::Part* part)
{
    ...
    if (ps)
        throw FEBioImport::RepeatedSurface(szname);
...
void FEBioMeshSection4::ParseElementSetSection(XMLTag& tag, FEBModel::Part* part)
{
    ...
    if (ps)
        throw FEBioImport::RepeatedElementSet(szname);
...
void FEBioMeshSection4::ParsePartListSection(XMLTag& tag, FEBModel::Part* part)
{
    ...
    if (ps)
        throw FEBioImport::RepeatedPartList(szname);
    ...
    FEBModel::ElementSet* es = new FEBModel::ElementSet("@part_list:" + std::string(szname));
    part->AddElementSet(es);

    std::vector<int> elemList;
    for (std::string s : partList)
    {
...
void FEBioMeshSection4::ParseEdgeSection(XMLTag& tag, FEBModel::Part* part)
{
    ...
    if (ps)
        throw FEBioImport::RepeatedEdgeSet(szname);
    ...
    // read edges
    std::vector<FEBModel::EDGE> edgeSet(edges);
    ++tag;
