...
void FEBioMeshDomainsSection4::Parse(XMLTag& tag)
{
    ...
    if (tag.isleaf() == false)
    {
        ...
        {
            if (tag == "SolidDomain")
                ParseSolidDomainSection(tag);
            else if (tag == "ShellDomain")
                ParseShellDomainSection(tag);
            else if (tag == "BeamDomain")
                ParseBeamDomainSection(tag);
            else
    ...
    FEBModel::Part* part = feb.GetPart(0);
    assert(part);
...
void FEBioMeshDomainsSection4::ParseSolidDomainSection(XMLTag& tag)
{
    ...
    if (partDomain == nullptr)
        throw XMLReader::InvalidAttributeValue(tag, "name", szname);

    // get the material name
    const char* szmat = tag.AttributeValue("mat");
    FEMaterial* mat = fem.FindMaterial(szmat);
    if (mat == nullptr)
        throw XMLReader::InvalidAttributeValue(tag, "mat", szmat);
