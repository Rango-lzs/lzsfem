...
{
    // Initialize traits for Tet10 element
    m_pTraits = RgElementTraitsStore::GetInstance()->GetElementTraits(FE_TET10G4);
...
{
    return ElementType::FE_TET10G4;  // 10-node tet with 4 Gauss points
}
...
ElementCategory RgTet10Element::elementCategory() const
{
...
void RgTet10Element::computeBMatrix(const NaturalCoord& naturalCoord, Matrix& B)
{
    ...
    // B matrix has shape [6, 30] for 3D solid element
    // 6 strain components (Voigt): {exx, eyy, ezz, exy, eyz, exz}
    // 30 DOFs: 3 per node * 10 nodes
