...
public:
    ...
    virtual RgMaterialPointData* createRgMaterialPointData() const = 0;

    /// Compute constitutive response for given material point
    /// Material uses kinematics stored in RgMaterialPointData to compute stress
    /// Constitutive matrix D is returned as an output parameter
    virtual void computeConstitutive(RgMaterialPoint* mp, Matrix& D) = 0;

    /// Commit state variables at material point
    virtual void commitState(RgMaterialPoint* mp) = 0;

    /// Revert state variables at material point to last committed state
    virtual void revertState(RgMaterialPoint* mp) = 0;
