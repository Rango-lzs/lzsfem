...
Matrix<double>& Quad2::stiffnessMatrix()
{
	...
	Matrix<double> dNdx(2, 4);
	Matrix<double> B_gp_lzs(3, 8);
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		//lzs
		Matrix<double> invJ = Jacobian.inverse();

		//dnds [r,s]
		//[dni/dr dni/ds] 2*4
		//dndx[x,y] = invJ*dnds
		//B [dn/dx]
		double r = GaussIntegration::getKsiGP(i);
		double s = GaussIntegration::getEtaGP(i);
		dNdx = inter->getdNdx(r,s);
		for (int i = 0; i < 4; ++i)
		{
			B_gp_lzs(0, 2 * i) = dNdx(0, i);
			B_gp_lzs(1, 2 * i + 1) = dNdx(1, i);

			B_gp_lzs(2, 2 * i) = dNdx(1, i);
			B_gp_lzs(2, 2 * i+1) = dNdx(0, i);
		}

		calculateG(G); //G: 4*8
		assembleA(A, Jacobian); //A: 3* 4
		B_gp = A * G;  //3*8
