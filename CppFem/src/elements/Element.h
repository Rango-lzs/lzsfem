...
namespace fem
{
	 ...
	 */
	class FEM_EXPORT Element : public FEMComponent
	{
	protected:
	...
	public:
		...
		virtual DofManager* giveInternalDofManager(int i) const {
			FEM_ERROR("No such DOF available on Element %d", number);
			return NULL;
		}
		//@}
		/**
		...
		{
			FEM_ERROR("Missing support for computing unknown vector at local element coordinates\n");
		}
		/**
		 ...
		 */
		virtual int giveIPValue(FloatArray& answer, GaussPoint* gp, TimeStep* tStep);
		int giveGlobalIPValue(FloatArray& answer, GaussPoint* gp, TimeStep* tStep);
		 ...
		 */
		virtual double giveCharacteristicLength(const FloatArray& normalToCrackPlane) { FEM_ERROR("Function not overloaded, which probably means that the crack band approach should not be used for this element"); return 0.; }
		/**
